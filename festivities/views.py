...
class FestivitiesViewSet(ModelViewSet):
  ...
  def list(self, request):
    ...
    if request.GET:
      if request.GET.get('name'):
        festivities = Festivity.objects.filter(
        ...
        festivities = Festivity.objects.filter(start_date__range=
          [request.GET['start_range'], request.GET['end_range']]
        )
      else:
  ...
  def create(self, request):
    ...
    if serialize_data.is_valid():
      ...
      return Response201('festivity', serialize_data.data, 
        user_message='Festivity created successfuly').get_response()
    else:
      server_info = serialize_data.errors
      return Response400(server_info, 
        user_message='Sorry. Festivity could not be created').get_response()
    ...
    if pk:
      ...
      if festivity:
        ...
        if serializer.is_valid(raise_exception=True):
          ...
          return Response200('festivity', serializer.data, 
            user_message='Festivity updated successfuly').get_response()
        else:
          server_info = serializer.errors
          return Response400(server_info, 
            user_message='Sorry. Festivity could not be updated').get_response()
      else:
